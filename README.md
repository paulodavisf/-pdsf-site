...
3. Para ativar o formulário, configure `SUPABASE_URL` e `SUPABASE_ANON_KEY` nas variáveis do projeto Vercel.
...
A função em `api/contact.js` valida os campos e grava os dados na tabela `contact_submissions`. Ela usa apenas a chave `anon`; não coloque `service_role` no site nem nas variáveis desta função.

...
module.exports = async function contact(req, res) {
  ...
  const submission = {
    ...
    phone: text(body.telefone, 40),
    service: text(body.assunto, 80) || null,
    message: text(body.mensagem, 4000),
    privacy_consent: body.consentimento === "sim",
  };

  const phone = normalizeBrazilPhone(submission.phone);
  if (!phone.valid) {
    return send(res, 422, {
      error: "Informe um telefone brasileiro válido com DDD.",
    });
  }
  submission.phone = phone.phone;
